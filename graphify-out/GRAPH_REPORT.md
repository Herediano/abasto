...
## Corpus Check
- 195 files · ~116,938 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1483 nodes · 3942 edges · 93 communities (67 shown, 8 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 149 edges (avg confidence: 0.85)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `76ef746b`
...
### Community 0 - "api.ts"
Cohesion: 0.06
Nodes (140): EmptyState(), ExportMenu(), download(), Field(), PermissionRoute(), FullScreenRoute(), ProtectedRoute(), PageHeader() (+132 more)
...
Cohesion: 0.16
Nodes (22): JwtAuthGuard, Injectable, TIMEZONES, AuthRequest, AuthUser, csvCell(), ExportColumn, sendExport() (+14 more)
...
Cohesion: 0.05
Nodes (46): CashRegistersController, CashShiftsController, Body, Controller, Get, Inject, Param, Post (+38 more)

### Community 7 - "products.controller.ts"
Cohesion: 0.09
Nodes (30): Decimalish, priceChange(), PriceField, PriceHistoryEntry, PriceSource, toNumber(), Db, guardarPrecio() (+22 more)
...
Cohesion: 0.06
Nodes (50): AccountList(), iniciales(), BranchSwitcher(), CommandPalette(), norm(), EscritorioShell(), PaletteContext, usePalette() (+42 more)
...
Cohesion: 0.18
Nodes (11): PricesController, Body, Controller, Delete, Get, Param, Post, Query (+3 more)

### Community 21 - "products table"
Cohesion: 0.29
Nodes (8): Auth Endpoints (signup/login/users), API Base Reference, Products Endpoints, Category Model (per-tenant), POST /products/import-reference, product_reference Global Table, GET /product-reference/:ean, products table
...
### Community 28 - "El escritorio"
Cohesion: 0.33
Nodes (6): Configurable, Dos excepciones, por contexto de trabajo, El escritorio, Estado tranquilo, La tarjeta — un solo molde para las ~14, Se arma por permiso
...
### Community 33 - "api"
Cohesion: 0.14
Nodes (21): api, iniciales(), PasswordSection(), submit(), PerfilSection(), submit(), PreferenciasSection(), pickStartup() (+13 more)
...
Cohesion: 0.22
Nodes (11): Graphify Knowledge Graph Workflow (AGENTS.md), main.ts Bootstrap (ValidationPipe), Graphify Knowledge Graph Workflow (CLAUDE.md), Multi-Tenancy Strategy, Mayorista ERP Project Overview, Purchase Invoice Lifecycle (draft/confirm/corrected), Append-only Stock Ledger, Manual Validation Style (no DTOs) (+3 more)
...
Cohesion: 0.15
Nodes (17): errorMessage(), fileToResizedDataUrl(), RangosPage(), abrirEdicion(), borrar(), crear(), guardar(), EmpresaSection() (+9 more)
...
Cohesion: 0.05
Nodes (28): BootstrapService, Inject, Injectable, TIPOS_MOVIMIENTO, Usuario, CuentasCorrientesService, Inject, Injectable (+20 more)
...
Cohesion: 0.25
Nodes (7): classify-reference-categories.ts script, enrich-reference-categories.ts script, Brand/Keyword Offline Classifier, Reference Catalog Category Classification, Precios Claros API (category tree), SEPA Open Data Source, Mayorista ERP README Overview
...
Cohesion: 0.18
Nodes (11): Deudas conocidas, Diseño — Abasto, El módulo — también un solo molde, El sistema visual, cerrado, Exportar — la salida universal, Hecho, IA — Ctrl + K, desde cualquier lado, Lo que sigue, en orden (+3 more)
...
- **Why does `AuthService` connect `AuthService` to `AuthRequest`, `.update`, `Mayorista ERP Project Overview`, `.token`?**
  _High betweenness centrality (0.264) - this node is a cross-community bridge._
- **Why does `AuthRequest` connect `AuthRequest` to `SalesService`, `AuthService`, `RequirePermission`, `products.controller.ts`, `StockService`, `PromotionsController`, `PurchasesService`, `.update`, `PricesController`, `PriceListsController`, `PriceRulesController`, `CustomersController`, `Tenants Controller`, `BranchesController`, `RangosService`, `CategoriesController`, `ReportesController`, `EscritorioController`, `WarehousesController`?**
  _High betweenness centrality (0.169) - this node is a cross-community bridge._
- **Why does `errorMessage()` connect `errorMessage` to `api.ts`, `ProductSearchDialog`, `api`, `CategoriesPage`, `SuppliersPage`, `ExpirationsPage`, `escritorio-page.tsx`, `money`, `.token`, `UsersPage`, `SucursalesSection`, `WarehousesPage`, `StockInPage`, `SalesHistoryPage`, `SupervisorAuthDialog`?**
  _High betweenness centrality (0.090) - this node is a cross-community bridge._
- **What connects `name`, `private`, `start:dev` to the rest of the system?**
  _280 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `api.ts` be split into smaller, more focused modules?**
  _Cohesion score 0.05708070030042474 - nodes in this community are weakly interconnected._
