  ...
  return (
    <>
      ...
      {loading ? (
        ...
        <EmptyState
          icon={Vault}
          title={activeFilters.length > 0 ? 'Sin turnos con estos filtros' : 'Todavía no hay turnos'}
          description={
            activeFilters.length > 0
              ? 'Probá quitando algún filtro.'
              : 'Los turnos se abren desde la pantalla de caja.'
          }
        />
      ) : (
        <div>
            ...
            <div className="flex flex-col gap-4 text-sm">
              ...
              {detalle.status === 'closed' ? (
                <div className="grid grid-cols-3 gap-3">
                  ...
                  <div className="rounded-md border border-border p-3">
                    <p className="text-chico text-placeholder">Contado</p>
                ...
                {!detalle.movements?.length ? (
                  ...
                  detalle.movements.map(m => (
                    <div key={m.id} className="flex justify-between">
                      <span>{m.reason} <span className="text-chico text-placeholder">· {m.userName}</span></span>
